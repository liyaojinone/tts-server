...
Each protocol service remains the boundary between this repository and the upstream engine. The service `start.ps1` scripts compute the workspace root, set `PYTHONPATH` for shared protocol packages plus the engine source, and expose the standard FastAPI app through `bobogen-service-kit`.
