...
from app.core.exceptions import (
    ...
    ProviderExternalStartRequiredError,
...
class ProcessManager:
    ...
    async def _is_healthy(self, provider_id: str) -> bool:
        try:
            return await self.healthcheck(provider_id)
        except Exception:
            return False
    ...
    async def _require_external_start(self, provider_id: str) -> ProviderRuntimeState:
        provider = self.providers[provider_id]
        state = self.get_state(provider_id)
        state.status = "starting"
        state.startup_attempts += 1
        ...
        compose_service = self._compose_service_name(provider_id)
        state.status = "failed"
        state.last_error = "external service is not running"
        raise ProviderExternalStartRequiredError(
            (
                f"Provider {provider_id} is external. Start it with: "
                f"bash start.sh --docker --model {compose_service} "
                f"or docker compose --profile {compose_service} up -d {compose_service}"
            ),
            {
                "provider_id": provider_id,
                "compose_service": compose_service,
                "start_command": f"bash start.sh --docker --model {compose_service}",
            },
        )

    def _compose_service_name(self, provider_id: str) -> str:
        if provider_id == "stable_audio_3_small_sfx":
            return "stable-audio3"
