...
通用本地和云端生成模型服务集合。核心思路：各模型服务（CosyVoice、F5-TTS、GPT-SoVITS、IndexTTS、VoxCPM、Stable Audio 3 等）作为独立子进程运行，由 `bobogen-gateway` 统一代理并暴露一致的 HTTP API；`bobogen-protocol` 定义共享 Pydantic 模型，`bobogen-service-kit` 提供 FastAPI 服务装配能力。

## 项目结构

```
bobogen-protocol/      共享协议 Pydantic 模型 (models.py)
bobogen-service-kit/   通用 FastAPI 服务装配 + 异常映射 + ProfileStore
bobogen-gateway/       统一网关 (app/adapters/、app/routers/、app/services/)
...
- 服务认证通过环境变量 `BOBOGEN_API_KEY` 控制，设置后所有请求需带 `Authorization: Bearer <key>` 头
