...
$payload = @{
    text = "你好，这是 GPT-SoVITS clone-test 脚本的测试。"
...
$payloadFile = Join-Path $env:TEMP "gptsovits-clone-payload.json"
$payload | Set-Content -Path $payloadFile -Encoding UTF8
$responseHeaders = Join-Path $env:TEMP "gptsovits-clone-headers.txt"
& curl.exe -sS `
    -D $responseHeaders `
