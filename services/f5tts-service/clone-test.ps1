param(
    ...
    [string]$Language = "zh",
    [string]$Emotion = "calm",
    [string]$Output = "f5tts-clone-test.wav"
)
...
$cloneResponseJson = & curl.exe -sS `
    -X POST "http://127.0.0.1:5102/v1/clone" `
...
$payload = @{
    text = "你好，这是 F5-TTS clone-test 脚本的测试。"
...
$payloadFile = Join-Path $env:TEMP "f5tts-clone-payload.json"
$payload | Set-Content -Path $payloadFile -Encoding UTF8
$responseHeaders = Join-Path $env:TEMP "f5tts-clone-headers.txt"
& curl.exe -sS `
    -D $responseHeaders `
    -H "Content-Type: application/json" `
    -o $Output `
    -X POST "http://127.0.0.1:5102/v1/synthesize" `
