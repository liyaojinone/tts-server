...
class F5TTSServiceCheckpointSelectionTest(unittest.TestCase):
    def test_prefers_official_v1_checkpoint_over_local_finetune_ckpts(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual("F5TTS_v1_Base", handler.MODEL_NAME)
            self.assertIn("models--SWivid--F5-TTS", str(ckpt_file))
            self.assertEqual("model_1250000.safetensors", ckpt_file.name)
